  ...
  try {
    ...
    if (integration.expiresAt && new Date() >= integration.expiresAt) {
      if (integration.refreshToken) {
        ...
        if (!accessToken) return NextResponse.json({ error: 'Failed to refresh access token' }, { status: 401 })
      } else {
    ...
    const updated = await client.api(`/me/events/${eventId}`).patch(updateBody)

    return NextResponse.json({ success: true, updated })
  } catch (error: any) {
    console.error('Focus stop API error:', error)
