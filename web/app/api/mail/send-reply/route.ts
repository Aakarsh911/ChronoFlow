...
/**
 * Send reply to email via Gmail or Outlook
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { emailId, provider, replyContent } = await request.json()

    if (!emailId || !provider || !replyContent || !['gmail', 'outlook'].includes(provider)) {
    ...
    if (provider === 'gmail') {
      ...
      console.log(`✅ Sent Gmail reply to message ${emailId}`)
      ...
      console.log(`✅ Sent Outlook reply to message ${emailId}`)
